...
if __name__ == "__main__":
	...
	BATCHSIZE = 50
	filePath = '../data/20190816/Dual/win17000/freq10/'
	...
	#       on Imac the GPU is not working. so
		with tf.device('/gpu:2'):
			...
			h_conv16 = tf.nn.relu(conv2d(h5, W_conv16) + b_conv16)
		...
		#Audio model
			...
			h4 = tf.nn.dropout(h_pool24, keep_prob)

			W_conv25 = weight_variable([1, 3, 32, 64])
			b_conv25 = bias_variable([64])
			h_conv25 = tf.nn.relu(conv2d(h4, W_conv25) + b_conv25)
...
#			y_conv4 = tf.nn.softmax(tf.matmul(y_conv3, W_conv4) + b_conv4)
		...
		for j in range(50001):
			...
			if j % BATCHSIZE*10 == 0:
				...
				test_accuracy = accuracy1.eval(feed_dict={acc_inputX: acc_xTest, acc_outputY: acc_yTest, aud_inputX: aud_xTest, aud_outputY: aud_yTest, keep_prob: 1.0})
		...
		yPreTmp = tf.argmax(y_conv1, 1)
		val_acc, yPred = sess.run([accuracy1, yPreTmp], feed_dict={acc_inputX: acc_xTest, acc_outputY: acc_yTest, aud_inputX: aud_xTest, aud_outputY: aud_yTest, keep_prob: 1.0})
